...
struct InputFieldLabelsView: View {
    ...
    @State private var generic1 = ""
    @State private var generic2 = ""
    @State private var generic3 = ""

    var body: some View {
        RuleScreen {
            // ---- Sub-rule A, AI-RAISED variant ------------------------------------------
            // mark_for_input_field_ai_review escalates only a deterministic PASS to
            // RuleStatus::AI_REVIEW; a FAIL keeps its own status and the AI merely attaches
            // ai_suggested_label / ai_reason. Every field in the cards BELOW is a
            // deterministic FAIL and therefore reports as a STATIC finding. For a finding the
            // AI itself raises, a field must PASS statically and still have a bad label.
            //
            // The iOS label rule FAILs on placeholder.present? for any accessible text field,
            // so an EMPTY placeholder is what makes these PASS and escalate. Their names are
            // then deliberately GENERIC, which the input-label judge scores 20-29 and reports.
            //
            // Keep them meaningless: naming them properly makes the model return not-an-issue
            // and collapse_input_field_rule_to_pass clears the finding entirely.
            //
            // This card is deliberately FIRST: the scan captures the viewport without
            // scrolling, so anything below the fold may never be seen at all.
            Card {
                caption("Field")
                TextField("", text: $generic1).inputBox().accessibilityLabel("Field")
                caption("Input", top: 14)
                TextField("", text: $generic2).inputBox().accessibilityLabel("Input")
                caption("Text box", top: 14)
                TextField("", text: $generic3).inputBox().accessibilityLabel("Text box")
            }
