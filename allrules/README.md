...
| 2 | `imageview-element-content-label` | ImageView with no contentDescription (iOS: label = stop word `image`) | both |
| 3 | `screen-reader-for-interactive-elements` | clickable ImageButton with no label (violated via its dependent rules) | both |
| 4 | `duplicate-element-content-label` | two ImageButtons both labelled "Add" | both |
| 5 | `button-element-content-label-capitalization` | Button "submit now" (lowercase first word) | both |
| 6 | `label-at-front` | Button text "Submit", label "Tap here to Submit" | both |
| 7 | `label-in-name` | Button text "Confirm", label "Send form" | both |
| 8 | `special-character-element-content-label` | label "Notifications 🔔" | both |
...
| 14 | `interactive-element-unsupported-type` | custom `FancyToggle extends View` (class outside android.*/androidx); iOS accessible element with zero traits | both |
...
| # | Rule ID | Fixture | Platforms |
|---|---|---|---|
| 16 | `text-color-contrast` | 14sp #999999 on #FFFFFF (2.85:1 < 4.5:1) | both |
...
| 21 | `text-truncation` | fixed-width 16sp single-line TextView, long text (needs characterLocations) | both |
| 22 | `responsive-containers` | fixed 140×24dp box with overflowing text — **page must support both orientations** | both |
...
| 25 | `overlapping-interactive-elements` | two sibling clickables with pixel-identical bounds | both |
| 26 | `two-dimensional-scroll` | custom view exposing all four scroll actions; iOS 2-axis ScrollView + `.isAdjustable` | both |
...
| # | Rule ID | Fixture | Platforms |
|---|---|---|---|
| 29 | `accessible-input-field-label` | caption TextView without `labelFor` + unnamed EditText | both |
...
| 37 | `missing-view-type-in-spoken-output` | custom `TapArea` clickable view, no role in spoken output (needs focus capture) | both |
